...
class OrdersCalendarWidget {
  constructor(params = {}) {
    // Сохраняем контекст для callback-функций
    const self = this;
  ...
  applySettings(newSettings) {
    if (newSettings.deal_date_field_id) {
      this.fieldIds.ORDER_DATE = newSettings.deal_date_field_id;
    }
    if (newSettings.delivery_range_field) {
      this.fieldIds.DELIVERY_RANGE = newSettings.delivery_range_field;
    }
